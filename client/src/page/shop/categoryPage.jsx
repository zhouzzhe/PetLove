...
function CategoryPage() {
    ...
    const [category, setcategory] = useState([]);  //全部分類資料
    const [categoryData, setCategoryData] = useState([]); //分類資料
    const [price, setPrice] = useState([]); //價格資料
    const { categoryPage } = useParams();
    const [categoryCurrentPage, setcategoryCurrentPage] = useState(0);  //paginate 從0開始
    const [categoryNumPage, setCategoryNumPage] = useState(1);  //總頁數
    ...
    useEffect(() => {
        const getCategoryPage = async () => {
            const categoryPageData = await axios.get(`http://localhost:8000/product/${categoryPage}/${categoryCurrentPage + 1}`);
            // console.log(categoryCurrentPage);
            setCategoryData(categoryPageData.data.data)
            setCategoryNumPage(categoryPageData.data.lastPage)  //總頁數
            //設定最高與最低價格
            const priceData = categoryPageData.data.data.map(productItem => {
    ...
    const handlePageClick = (event) => {
        setcategoryCurrentPage(event.selected)
    }
        ...
        <div className='bodyMargin '>
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                ...
                <Modal.Body>
                    <div className="itemSelect col-6 mt-4 ">
                        <div>
                            <h5>{categoryData.product_name}</h5>
                            <p>選擇規格</p>
            ...
            <div className='d-flex positon-absolute'>
                ...
                <div className="cardbox d-flex flex-wrap justify-content-center align-items-center">
                    {categoryData.map((productItem, index) =>
                    ...
                    <div className="d-flex justify-content-center align-items-center">
                        <ReactPaginate
                            ...
                            pageCount={categoryNumPage} //指定總頁數
